 ...
 *
 * Features:
 * - Pitch servo to adjust up/down angle
 * - Flywheel motor to launch FUEL
 * - Trajectory calculations for automated shooting
 */
public class Shooter extends SubsystemBase {
    ...
    public void setPitchAngle(double angleDegrees) {
        ...
        targetPitchAngle = Math.max(ShooterConstants.PITCH_MIN_ANGLE,
    ...
    /**
     * Check if the pitch servo is approximately at the target angle.
     * Note: Standard servos don't provide position feedback, so this
     * assumes the servo reaches position after a brief delay.
     *
    ...
    public Command spinUpCommand() {
        return Commands.sequence(
            Commands.runOnce(this::spinUp, this),
            Commands.waitUntil(this::isFlywheelAtSpeed)
        ).withName("Spin Up Shooter");
    }

    /**
     * Command to prepare shooter with automatic pitch calculation.
     * Uses trajectory calculation to determine optimal pitch angle.
     */
    ...
    /**
     * Command to prepare shooter using vision-assisted targeting.
     * Uses AprilTags for more precise distance/angle calculation.
     * Falls back to odometry if no tags visible.
     */
    ...
    /**
     * Command to continuously track hub with vision and update pitch.
     * Run this while waiting for a shot opportunity.
     */
    public Command trackHubCommand() {
        ...
        }, this).withName("Track Hub");
    }

    /**
     * Command to spin up and aim using vision, then wait for driver trigger.
     * Continuously updates pitch based on AprilTag data.
     */
    ...
    private void UpdateHubLocation() {
        ...
        hub[0] = ((DriverStation.getAlliance().equals(Alliance.Blue)) ? ShooterConstants.BLUE_X
                : ((DriverStation.getAlliance().equals(Alliance.Red)) ? ShooterConstants.RED_X
                        : -180.0));
    }
    ...
    /**
     * Calculate optimal pitch angle for current position.
     ...
     * when AprilTags are visible. Falls back to odometry-based calculation if
     * vision data is unavailable or unreliable.
     *
    ...
    /**
     * Calculate pitch angle from horizontal distance using interpolation.
    ...
    /**
     * Calculate pitch angle using physics-based projectile motion.
     * Used when we have high-confidence vision data.
     *
    ...
    private double calculatePitchPhysics(double horizontalDistance, double verticalDistance) {
        ...
        // Clamp to valid range
        return Math.max(ShooterConstants.PITCH_MIN_ANGLE,
                Math.min(ShooterConstants.PITCH_MAX_ANGLE, theta));
    }
